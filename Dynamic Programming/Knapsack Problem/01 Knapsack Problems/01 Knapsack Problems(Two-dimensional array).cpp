...
inline void DP(){
	for(int i = 1;i <= ThingsNumber;i++)
		for(int j = BagWeight;j > 0;j--)
			if(Things[i].Weight <= j) Answer[i][j] = Max(Answer[i - 1][j],Answer[i - 1][j - Things[i].Weight] + Things[i].Value);
...
//	printf("%d%d",BagWeight,ThingsNumber);
	...
	DP();
/*	for(register int i=0;i<=ThingsNumber;i++){
		std::cout<<"ThingsNumber:"<<i<<std::endl;
